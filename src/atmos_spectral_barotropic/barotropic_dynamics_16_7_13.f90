...
beta_star=>NULL(), tester=>NULL(), tester2=>NULL(), tester3=>NULL(), vormean_x=>NULL(), vormean_y=>NULL(), vorprime_x=>NULL(), vorprime_y=>NULL(), vormean_y_tend=>NULL(), vor_y=>NULL(), vor_x=>NULL(), &
 fourier_u=>NULL(), vorprime_prev=>NULL(), vorprime=>NULL(), vork=>NULL(), vork_prev=>NULL(), vork_curr=>NULL(), vortendk=>NULL(), ucurr=>NULL(), vcurr=>NULL(), &
! Energy
...
real,    dimension(is:ie, js:je)  :: enstrophytendcheck, vortendmean, pvor_advecmean, rvor_advecmean, vortruncmean, vorlindamp_emean, vorlindamp_mmean, vorspecdampmean, vorstirmean, vorrobdampmean, &
				     ...
				     enstrophytrunc_mean, enstrophytrunc_prime, enstrophystir_mean, enstrophystir_prime
...
complex, dimension(is:ie/2+1, js:je) :: vortendkII, dt_vork
...
complex, dimension(is:ie+1, js:je):: uk, vk, vork, vork_curr, vork_prev, vortendk, enstrophy_tendk, vbetak, vdy_vormeank, enstrophy_pvor_adveck, enstrophy_rvormean_adveck, vortrunck, vorlindamp_ek, vorlindamp_mk, vorspecdampk, vorrobdampk, vorstirk, vor_xk, vor_yk, vor_xkII, dvork, vor_ykII, enstrophy_trunck, enstrophy_lindamp_ek, enstrophy_lindamp_mk, enstrophy_specdampk, enstrophy_robdampk, enstrophy_stirk, triple_eddy_sumpq, dt_vorkII
complex, dimension(is:ie, is:ie, is:ie, js:je) :: triple_eddy
...
enstrophyf_curr = fft_grid_to_fourier(Dyn%Grid%vor(:,:,current))*conjg(fft_grid_to_fourier(Dyn%Grid%vor(:,:,current)))

 Dyn%Grid%fourier_enstrophy(1:ie/2+1,:,current) = real(enstrophyf_curr)
 Dyn%grid%fourier_enstrophy(ie/2+2:ie,:,current) = aimag(enstrophyf_curr(2:ie/2-1,:))

vork_curr(1:ie/2+1,:) = fft_grid_to_fourier(Dyn%grid%vor(:,:,current))
vork_curr(ie/2+2:ie+1,:) = conjg(vork_curr(ie/2+1:2:-1,:))

!rubf = conjg(fft_grid_to_fourier(Dyn%grid%v(:,:,current)))
!vf_curr(is:ie/2,:) = rubf(ie:is-1:-1,:)
!vf_curr(ie/2+1:ie,:) = fft_grid_to_fourier(Dyn%grid%v(:,:,current)) ! Makes vf_curr look like [negative freq; 0 freq; positive freq]
!
!rubf = conjg(fft_grid_to_fourier(Dyn%grid%vor(:,:,current)))
!vorf_curr(is:ie/2,:) = rubf(ie:is-1:-1,:)
!vorf_curr(ie/2+1:ie,:) = fft_grid_to_fourier(Dyn%grid%vor(:,:,current))
...
! call trans_spherical_to_grid(dt_vors_rub, A)
! call vor_div_from_uv_grid (zerog, -Dyn%Grid%u(:,:,current), dt_vors_rub, dt_divs_rub)  
! call trans_spherical_to_grid(dt_vors_rub, B)

 call trans_grid_to_spherical(Dyn%Grid%vor(:,:,current), vors_rub)
 call horizontal_advection(vors_rub, Dyn%Grid%u(:,:,current), Dyn%Grid%v(:,:,current), Dyn%Grid%tester3(:,:,future))
...
vortendkII = fft_grid_to_fourier(Dyn%Grid%vortend(:,:,future))
!print *, "VORTENDKII", vortendkII(6,47)
...
!print *, "Dyn%Grid%fourier_enstrophy FUTURE", Dyn%Grid%fourier_enstrophy(7,47,future)
!print *, "enstrophyf FUTURE", enstrophyf(7,47)
!print *, "Dyn%Grid%fourier_enstrophy CURRENT", Dyn%Grid%fourier_enstrophy(7,47,current)
!print *, "enstrophyf CURRENT", enstrophyf_curr(7,47)
...
!print *, "Dyn%Grid%fourier_enstrophy_tend", Dyn%Grid%fourier_enstrophy_tend(1:5,47,future), Dyn%Grid%fourier_enstrophy_tend(65+1:65+5,47,future)
!print *, "enstrophyf_tend", enstrophyf_tend(1:5,47)
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
! Because the above product rule demands knowledge of u @ t=n+1 I calculate all the terms at the end of the subroutine, when this value is known.
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Dyn%grid%vorprime(:,:,future) = vorprime
Dyn%grid%vorprime_prev(:,:,future) = vorprime_prev
Dyn%Grid%enstrophy_tend(:,:,future) = Dyn%Grid%vortend(:,:,future)*(vorprime_prev+vorprime)
...
Dyn%Grid%enstrophy_dt_vor(:,:,future) = Dyn%Grid%dt_vor(:,:,future)*(vorprime_prev+vorprime)
!print *, "ENSTROPHY_DTVOR", Dyn%Grid%enstrophy_dt_vor(22,47,future)
Dyn%Grid%enstrophy_pvor_advec(:,:,future) = Dyn%Grid%pvor_advec(:,:,future)*(vorprime_prev+vorprime)
!print *, "ENSTROPHY_PVORADVEC", Dyn%Grid%enstrophy_pvor_advec(22,47,future)
Dyn%Grid%enstrophy_rvormean_advec(:,:,future) = Dyn%Grid%rvormean_advec(:,:,future)*(vorprime_prev+vorprime)
!print *, "ENSTROPHY_RVORMEAN_ADVEC", Dyn%Grid%enstrophy_rvormean_advec(22,47,future)
Dyn%Grid%enstrophy_rvorprime_advec(:,:,future) = Dyn%Grid%rvorprime_advec(:,:,future)*(vorprime_prev+vorprime)
!print *, "ENSTROPHY_RVORPRIME_ADVEC", Dyn%Grid%enstrophy_rvorprime_advec(22,47,future)
Dyn%Grid%enstrophy_trunc(:,:,future) = Dyn%Grid%vortrunc(:,:,future)*(vorprime_prev+vorprime)
!print *, "ENSTROPHY_TRUNC", Dyn%Grid%enstrophy_trunc(22,47,future)
Dyn%Grid%enstrophy_lindamp_m(:,:,future) = Dyn%Grid%vorlindamp_m(:,:,future)*(vorprime_prev+vorprime)
!print *, "ENSTROPHY_LINDAMP_M", Dyn%Grid%enstrophy_lindamp_m(22,47,future)
Dyn%Grid%enstrophy_lindamp_e(:,:,future) = Dyn%Grid%vorlindamp_e(:,:,future)*(vorprime_prev+vorprime)
!print *, "ENSTROPHY_LINDAMP_E", Dyn%Grid%enstrophy_lindamp_e(22,47,future)
Dyn%Grid%enstrophy_specdamp(:,:,future) = Dyn%Grid%vorspecdamp(:,:,future)*(vorprime_prev+vorprime)
!print *, "ENSTROPHY_SPECDAMP", Dyn%Grid%enstrophy_specdamp(22,47,future)
Dyn%Grid%enstrophy_stir(:,:,future) = Dyn%Grid%vorstir(:,:,future)*(vorprime_prev+vorprime)
...
Dyn%Grid%tester(:,:,future) = Dyn%Grid%rvormean_advec(:,:,future)!*(vorprime_prev+vorprime)
Dyn%Grid%tester2(:,:,future) = (Dyn%Grid%v(:,:,current)*Dyn%Grid%vormean_y(:,:,future) + Dyn%Grid%u(:,:,current)*Dyn%Grid%vormean_x(:,:,future))!*(vorprime_prev + vorprime)
...
vork(1:ie/2,:) = fft_grid_to_fourier(Dyn%grid%vor(:,:,future))
...
if(previous == current) then
vortendk = (vork-vork_curr)/delta_t
else
vortendk = (vork-vork_prev)/delta_t
endif

!print *, "VORTENDK", vortendk(1:10,47)
!print *, "VORTENDK DIFF", vortendk(1:10,47)-vortendkII(1:10,47)


enstrophy_tendk = ie**2*vortendk*(conjg(vork_prev) + conjg(vork))
enstrophy_pvor_adveck = ie**2*vbetak*conjg(vork)
enstrophy_rvormean_adveck = ie**2*vdy_vormeank*conjg(vork)
enstrophy_trunck = ie**2*vortrunck*conjg(vork)
enstrophy_lindamp_ek = ie**2*vorlindamp_ek*conjg(vork)
enstrophy_lindamp_mk = ie**2*vorlindamp_mk*conjg(vork)
enstrophy_specdampk = ie**2*vorspecdampk*conjg(vork)
enstrophy_robdampk = ie**2*vorrobdampk*conjg(vork)
enstrophy_stirk = ie**2*vorstirk*conjg(vork)

Dyn%grid%vortendk(1:ie/2+1,:,future) = real(vortendk(1:ie/2+1,:))
Dyn%grid%vortendk(ie/2+2:ie,:,future)= aimag(vortendk(2:ie/2,:))
...
!                      triple_eddy(p,q,k,:) = uk(p,:)*vor_xk(q,:)*conjg(vork(k,:)) + vk(p,:)*vor_yk(q,:)*conjg(vork(k,:))
!            	  else
!                      triple_eddy(p,q,k,:) = (0,0)
!                  end if
!            end do
!      end do
!end do
!
!triple_eddy_sumpq = sum(sum(triple_eddy(:,:,:,:),2),1)
!
!Dyn%grid%tester(1:ie/2+1,:,future) = real(triple_eddy_sumpq(1:ie/2+1,:))
!Dyn%grid%tester(ie/2+2:ie,:,future)= aimag(triple_eddy_sumpq(2:ie/2,:))
...
! SPECTRAL VORTICITY
!print *, "BUDGET: VORTENDK = PVOR_ADVECK + RVOR_ADVECK - VORTRUNCK", vortendkII(6,47) - dt_vorkII(6,47) - vortrunck(6,47)
! SPECTRAL ENSTROPHY
!print *, "SPECTRAL ENSTROPHY_TEND", enstrophyf_tend(7,47)*ie/2
!print *, "v'vor'beta*", vorfluxBf_curr(7,47)*ie
!print *, "v'vor'vor'", dI_dy(7,47)/2
!print *, "SPECTRAL ENSTROPHY_TRUNC", -enstrophy_truncf(7,47)*ie
!print *, "SPECTRAL ENSTROPHY_STIR", enstrophy_stirf(7,47)*ie
!print *, "ENST_TEND = -VORFLUX - 3xEddy - ENSTRUNC", enstrophyf_tend(7,47)*ie/2 +vorfluxBf_curr(7,47)*ie + dI_dy(7,47)/2 !- enstrophy_truncf(1,47)*ie + enstrophy_stirf(7,47)*ie
