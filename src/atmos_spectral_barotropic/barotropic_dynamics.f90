...
beta_star=>NULL(), tester=>NULL(), tester2=>NULL(), tester3=>NULL(), vormean_x=>NULL(), vormean_y=>NULL(), vorprime_x=>NULL(), vorprime_y=>NULL(), vormean_y_tend=>NULL(), vor_y=>NULL(), vor_x=>NULL(), &
 fourier_u=>NULL(), vorprime_prev=>NULL(), vorprime_curr=>NULL(), vorprime=>NULL(), vork=>NULL(), vork_prev=>NULL(), vork_curr=>NULL(), vortendk=>NULL(), vormeantend=>NULL(), vorprimetend=>NULL(), &
! Some more variables
uprev=>NULL(), ucurr=>NULL(), vprev=>NULL(), vcurr=>NULL(), vorprev=>NULL(), vorcurr=>NULL(), u_dvordx=>NULL(), v_dvordy=>NULL(), vor_dudx=>NULL(), vor_dvdy=>NULL(), &
! Energy
...
enstrophy_robdampk=>NULL(), enstrophy_stirk=>NULL(), &
! Spectral Fluxes
Ik=>NULL(), &
! Spectral Vorticity 
...
namelist /barotropic_dynamics_nml/ check_fourier_imag, south_to_north, &
                                 ...
                                 eddy_lat, eddy_width, zeta_0, m_0, damp, spec_deriv, mult, linmult, tau, tau1, read_file, &
                                 valid_range_v
...
allocate (Dyn%Grid%tester2		(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid%tester3		(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid%u_dvordx		(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid%v_dvordy		(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid%vor_dudx		(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid%vor_dvdy		(is:ie, js:je, num_time_levels))
...
! Spectral Fluxes
allocate (Dyn%grid%Ik				(is:ie, js:je, num_time_levels))
...
real,    dimension(is:ie, js:je)  :: dt_vorg, stir, uprime, vormean, vorprime, energyprev, energymean_prev, fourier_enstrophy_prev, A, B
...
complex, dimension(is:ie/2+1, js:je) :: dt_vork
real,    dimension(is:ie)	  :: xlength
real,    dimension(js:je-2)	  :: dlat
complex, dimension(is:ie/2+1, js:je):: vorf, enstrophyf, vorf_tend, enstrophyf_tend, vorf_prev, enstrophyf_prev, enstrophyf_curr, enstrophy_truncf, enstrophy_stirf, vorfluxBf_curr, fourier_rub, 					       vf, rubf, Ik
complex, dimension(is:ie/2+1, js:je):: vorf_curr, vf_curr
complex, dimension(is:ie/2+1, is:ie/2+1, js:je):: I_vor
complex, dimension(is:ie/2+1, is:ie/2+1, is:ie/2+1, js:je-2):: I
complex, dimension(is:ie/2+1, js:je-2) :: dI, dI_dy
...
complex, dimension(is:ie+1, js:je):: vortendk, enstrophy_tendk, vbetak, vdy_vormeank, vortrunck, vorlindamp_ek, vorlindamp_mk, vorspecdampk, vorrobdampk, vorstirk, vor_xk, vor_yk, vor_xkII, dvork, vor_ykII, enstrophy_trunck, enstrophy_lindamp_ek, enstrophy_lindamp_mk, enstrophy_specdampk, enstrophy_robdampk, enstrophy_stirk, triple_eddy_sumpq, dt_vorkII
...
enstrophyprev = vorprev*vorprev
enstrophyf_prev = fft_grid_to_fourier(Dyn%Grid%vor(:,:,previous))*conjg(fft_grid_to_fourier(Dyn%Grid%vor(:,:,previous)))
...
!print *, "VQ", Dyn%Grid%vq(40,40,future)
!print *, "-UQ", -Dyn%Grid%uq(40,40,future)
...
! PRODUCT RULE
! dU_dX
 call vor_div_from_uv_grid (zerog, Dyn%Grid%u(:,:,current), vors_rub, dt_divs_rub)
! call vor_div_from_uv_grid (zerog, Dyn%Grid%u(:,:,current), dt_vors_rub, dt_divs_rub) 
 call trans_spherical_to_grid(vors_rub, Dyn%Grid%vor_dudx(:,:,future))
 Dyn%Grid%vor_dudx(:,:,future) = vorprime_curr*Dyn%Grid%vor_dudx(:,:,future)
! dV_dY
 call vor_div_from_uv_grid (-Dyn%Grid%v(:,:,current), zerog, vors_rub, dt_divs_rub) 
! call vor_div_from_uv_grid (-Dyn%Grid%v(:,:,current), zerog, dt_vors_rub, dt_divs_rub) 
 call trans_spherical_to_grid(vors_rub, Dyn%Grid%vor_dvdy(:,:,future))
 Dyn%Grid%vor_dvdy(:,:,future) = vorprime_curr*Dyn%Grid%vor_dvdy(:,:,future)
! dVOR_dX
 call vor_div_from_uv_grid (zerog, vorprime_curr, vors_rub, dt_divs_rub)  
! call vor_div_from_uv_grid (zerog, Dyn%Grid%vor(:,:,current), dt_vors_rub, dt_divs_rub) 
 call trans_spherical_to_grid(vors_rub, Dyn%Grid%u_dvordx(:,:,future)) 
 Dyn%Grid%u_dvordx(:,:,future) = Dyn%Grid%u(:,:,current)*Dyn%Grid%u_dvordx(:,:,future)
! dVOR_dY
 call vor_div_from_uv_grid (-vorprime_curr, zerog, vors_rub, dt_divs_rub)
! call vor_div_from_uv_grid (-Dyn%Grid%vor(:,:,current), zerog, dt_vors_rub, dt_divs_rub) 
 call trans_spherical_to_grid(vors_rub, Dyn%Grid%v_dvordy(:,:,future))
 Dyn%Grid%v_dvordy(:,:,future) = Dyn%Grid%v(:,:,current)*Dyn%Grid%v_dvordy(:,:,future)
! TOTAL RELATIVE VORTICITY ADVECTION (JUST THE SUM OF MEAN + PRIME)
...
!print *, "METHOD 1: U.GRAD(f)", Dyn%Grid%tester(40,40,future)
! METHOD 2
! call vor_div_from_uv_grid (Dyn%grid%vor(:,:,current)*Dyn%Grid%v(:,:,current), -Dyn%grid%vor(:,:,current)*Dyn%Grid%u(:,:,current), dt_vors_rub, dt_divs_rub) 
! call trans_spherical_to_grid(dt_vors_rub, Dyn%Grid%rvor_advec(:,:,future))
!print *, "METHOD 2: U.GRAD(VOR)", Dyn%Grid%rvor_advec(40,40,future)
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

if(spec_deriv == 0) then
...
! call trans_spherical_to_grid(dt_vors,Dyn%Grid%tester(:,:,future))
...
Dyn%Grid%dy_u2_v2(:,:,future) = Dyn%Tend%v - Dyn%Grid%dy_u2_v2(:,:,current) - Dyn%Grid%vtrunc(:,:,future)
 ...
 call trans_grid_to_spherical(Dyn%Grid%u_dvordx(:,:,future) + Dyn%Grid%vor_dudx(:,:,future) + Dyn%Grid%v_dvordy(:,:,future) + Dyn%Grid%vor_dvdy(:,:,future) + &
				Dyn%Grid%pvor_advec(:,:,future) + Dyn%Grid%rvormean_advec(:,:,future),dt_vors)
 dt_vors = -2*dt_vors
! call trans_spherical_to_grid(dt_vors,Dyn%Grid%tester6(:,:,future))
 call uv_grid_from_vor_div(dt_vors,  zeros, Dyn%Grid%dx_u2_v2(:,:,current), Dyn%Grid%dy_u2_v2(:,:,current))
 Dyn%Grid%dx_u2_v2(:,:,future) = Dyn%Tend%u - Dyn%Grid%dx_u2_v2(:,:,current) - Dyn%Grid%utrunc(:,:,future)
 Dyn%Grid%dy_u2_v2(:,:,future) = Dyn%Tend%v - Dyn%Grid%dy_u2_v2(:,:,current) - Dyn%Grid%vtrunc(:,:,future)
endif
...
if(damp > 1) then ! do linear damping
 ...
 !print *, "V (EDDY) LINDAMP", Dyn%Grid%vlindamp(40,40,future)
 !print *, "U EDDY LINDAMP", Dyn%Grid%ulindamp_e(40,40,future)
 !print *, "U MEAN LINDAMP", Dyn%Grid%ulindamp_m(40,40,future)
 ...
 !print *, "VOR EDDY LINDAMP", Dyn%grid%vorlindamp_e(40,40,future) 
 !print *, "VOR MEAN LINDAMP", Dyn%grid%vorlindamp_m(40,40,future) 
endif
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!print *, "UTRUNC", Dyn%grid%utrunc(40,40,future)
!print *, "VTRUNC", Dyn%grid%vtrunc(40,40,future)
...
!print *, "VORTRUNC", Dyn%grid%vortrunc(40,40,future)
else
...
!print *, "USPECDAMP", Dyn%grid%uspecdamp(40,40,future)
!print *, "VSPECDAMP", Dyn%grid%vspecdamp(40,40,future)
!print *, "VORSPECDAMP", Dyn%grid%vorspecdamp(40,40,future)
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!print *, "U STIR", Dyn%grid%ustir(40,40,future)
!print *, "V STIR", Dyn%grid%vstir(40,40,future)
!print *, "VOR STIR", Dyn%grid%vorstir(40,40,future)
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!print *, "UROBDAMP", Dyn%grid%urobdamp(40,40,future)
!print *, "VROBDAMP", Dyn%grid%vrobdamp(40,40,future)
!print *, "VORROBDAMP", Dyn%grid%vorrobdamp(40,40,future)
...
do j = js, je 
 ...
 enstrophyf(:,:) = fft_grid_to_fourier(Dyn%Grid%vor(:,:,future))*conjg(fft_grid_to_fourier(Dyn%Grid%vor(:,:,future))) ! gives enstrophy in spectral space
 vorf(:,:) = fft_grid_to_fourier(Dyn%grid%vor(:,:,future))
 Dyn%Grid%fourier_enstrophy(1:ie/2+1,:,future) = real(enstrophyf)
 Dyn%grid%fourier_enstrophy(ie/2+2:ie,:,future) = aimag(enstrophyf(2:ie/2-1,:))
...
!print *, "ENERGY", Dyn%Grid%energy(40,40,future)
Dyn%Grid%energy_mean(:,:,future) = (umean**2+vmean**2)
Dyn%Grid%enstrophy(:,:,future) = Dyn%Grid%vor(:,:,future)*Dyn%Grid%vor(:,:,future)
!print *, "ENSTROPHY", Dyn%Grid%enstrophy(22,47,future)
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

! ------------------------------ SPECTRAL TRIPLE EDDY PRODUCT ------------------------------
os = 1
oe = ie/2+1
...
!print *, "TEST", sum(exp(2*pi*(0,1)*3*xlength/ie)*exp(2*pi*(0,1)*-3*xlength/ie)*exp(-2*pi*(0,1)*0*xlength/ie))

!do p= -ie,ie
!   do q= -ie,ie
!         print *, k
!         modes = sum(exp(2*pi*(0,1)*p*xlength/ie)*exp(-2*pi*(0,1)*q*xlength/ie)*exp(-2*pi*(0,1)*k*xlength/ie))
!	 I_vor(p+1,q+1,:) = vf(p+1,:)*vorf_curr(q+1,:)*modes 
!	 I(k+ie+1,p+ie+1,q+ie+1,:) = vf_curr(p+ie+1,:)*vorf_curr(q+ie+1,:)*conjg(vorf_curr(k+ie+1,:))*modes;
!	 if (p+q==0) then ! selection rule
!	 I(p+ie+1,q+ie+1,:) = vf_curr(p+ie+1,:)*vorf_curr(q+ie+1,:)*conjg(vorf_curr(ie+1,:))*ie;
!	 else 
!	 I(p+ie+1,q+ie+1,:) = (0,0)
!	 end if
!      end do
...
!	end if
!	 I_vor(p+1,q+1,:) = vf(p+1,:)*vorf_curr(q+1,:)*modes 
!	 I(k+1,p+1,q+1,:) = vf_curr(p+1,:)*vorf_curr(q+1,:)*conjg(vorf_curr(k+1,:))*modes;
!	 if (p+q==k) then ! selection rule
!         I(k+1,p+1,q+1,:) = vf_curr(p+1,:)*vorf_curr(q+1,:)*conjg(vorf_curr(k+1,:))*ie; ! Extra factor of ie due to integrating over fourier modes
!	 else 
!	 I(k+1,p+1,q+1,:) = (0,0)
!	end if
!     end do
...
!end do
!
! Take meridional derivative using central differences
!
!Ik = sum(sum(I(:,:,:,:),3),2)
...
!
!do j = is, ie/2+1 
!  dI(j,:) = cshift(Ik(j,:)*cos(deg_lat(:)*pi/180),1) - cshift(Ik(j,:)*cos(deg_lat(:)*pi/180),-1)
!  dI_dy(j,:) = (((radius*cos(deg_lat(:)*pi/180))**-1)*dI(j,:))/dlat
!end do

!print *, dI(1:5,5)
!print *, dI_dy

!Dyn%grid%Ik(1:ie/2+1,:,future) = real(dI_dy)
!Dyn%grid%Ik(ie/2+2:ie,:,future) = aimag(dI_dy(2:ie/2-1,:))
!Dyn%grid%Ik(1:ie/2+1,:,future) = real(dI)
!Dyn%grid%Ik(ie/2+2:ie,:,future) = aimag(dI(2:ie/2-1,:))
...
enstrophytendcheck(:,:) = 0.5*(Dyn%Grid%enstrophy(:,:,future)-Dyn%Grid%enstrophy(:,:,current))/delta_t
...
!enstrophyf_tend(:,:) = (enstrophyf-enstrophyf_curr)/delta_t
!vorf_tend(:,:) = (vorf-vorf_curr)/delta_t
else
...
enstrophytendcheck(:,:) = 0.5*(Dyn%Grid%enstrophy(:,:,future)-enstrophyprev)/delta_t ! See above comment
...
!enstrophyf_tend(:,:) = (enstrophyf-enstrophyf_prev)/delta_t
!vorf_tend(:,:) = (vorf-vorf_prev)/delta_t
endif
!print *, "UTEND", Dyn%Grid%utend(40,40,future)
!print *, "VTEND", Dyn%Grid%vtend(40,40,future)
print *, "VORTEND", Dyn%Grid%vortend(40,40,future)
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!print *, "ENERGY_TEND", Dyn%Grid%energy_tend(40,40,future)
...
!print *, "ENERGY_VORADVEC", Dyn%Grid%energy_voradvec(40,40,future)
...
!print *, "ENERGY_GRADTERM", Dyn%Grid%energy_gradterm(40,40,future)
...
!print *, "ENERGY_TRUNC", Dyn%Grid%energy_trunc(40,40,future)
Dyn%Grid%energy_lindamp_m(:,:,future) = Dyn%Grid%ulindamp_m(:,:,future)*(uprev(:,:)+Dyn%Grid%u(:,:,future))
!print *, "ENERGY_LINDAMP_M", Dyn%Grid%energy_lindamp_m(40,40,future)
...
!print *, "ENERGY_LINDAMP_E", Dyn%Grid%energy_lindamp_e(40,40,future)
...
!print *, "ENERGY_ROBDAMP", Dyn%Grid%energy_robdamp(40,40,future)
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!Dyn%Grid%energy_mean(:,:,future) = (umean**2+vmean**2)
!print *, "ENERGY_MEAN", Dyn%Grid%energy_mean(40,40,future)
!if(previous == current) then
!energytendcheck_mean(:,:) = (Dyn%Grid%energy_mean(:,:,future)-Dyn%Grid%energy_mean(:,:,current))/delta_t
!else
...
Dyn%Grid%enstrophy_tend(:,:,future) = 0.5*Dyn%grid%vortend(:,:,future)*(vorprime_prev+vorprime)
!Dyn%Grid%enstrophy_tend(:,:,future) = Dyn%grid%vorprimetend(:,:,future)*vorprime_curr
...
Dyn%Grid%enstrophy_pvor_advec(:,:,future) = Dyn%Grid%pvor_advec(:,:,future)*0.5*(vorprime_prev+vorprime)
print *, "ENSTROPHY_PVORADVEC", Dyn%Grid%enstrophy_pvor_advec(22,47,future)
Dyn%Grid%enstrophy_rvormean_advec(:,:,future) = Dyn%Grid%rvormean_advec(:,:,future)*0.5*(vorprime_prev+vorprime)
print *, "ENSTROPHY_RVORMEAN_ADVEC", Dyn%Grid%enstrophy_rvormean_advec(22,47,future)
if(spec_deriv == 0) then
Dyn%Grid%enstrophy_rvorprime_advec(:,:,future) = Dyn%Grid%rvorprime_advec(:,:,future)*0.5*(vorprime_prev+vorprime)
print *, "ENSTROPHY_RVORPRIME_ADVEC", -Dyn%Grid%enstrophy_rvorprime_advec(22,47,future)
else
Dyn%Grid%enstrophy_rvorprime_advec(:,:,future) = 0.5*(Dyn%Grid%u_dvordx(:,:,future) + Dyn%Grid%vor_dudx(:,:,future) + Dyn%Grid%v_dvordy(:,:,future) + Dyn%Grid%vor_dvdy(:,:,future))*(vorprime_prev+vorprime)
print *, "ENSTROPHY_RVORPRIME_ADVEC_PRODRULE", -Dyn%Grid%enstrophy_rvorprime_advec(22,47,future)
endif
Dyn%Grid%enstrophy_trunc(:,:,future) = Dyn%Grid%vortrunc(:,:,future)*0.5*(vorprime_prev+vorprime)
print *, "ENSTROPHY_TRUNC", Dyn%Grid%enstrophy_trunc(22,47,future)
Dyn%Grid%enstrophy_lindamp_m(:,:,future) = Dyn%Grid%vorlindamp_m(:,:,future)*0.5*(vorprime_prev+vorprime)
!print *, "ENSTROPHY_LINDAMP_M", Dyn%Grid%enstrophy_lindamp_m(22,47,future)
Dyn%Grid%enstrophy_lindamp_e(:,:,future) = Dyn%Grid%vorlindamp_e(:,:,future)*0.5*(vorprime_prev+vorprime)
!print *, "ENSTROPHY_LINDAMP_E", Dyn%Grid%enstrophy_lindamp_e(22,47,future)
Dyn%Grid%enstrophy_specdamp(:,:,future) = Dyn%Grid%vorspecdamp(:,:,future)*0.5*(vorprime_prev+vorprime)
!print *, "ENSTROPHY_SPECDAMP", Dyn%Grid%enstrophy_specdamp(22,47,future)
Dyn%Grid%enstrophy_stir(:,:,future) = Dyn%Grid%vorstir(:,:,future)*0.5*(vorprime_prev+vorprime)
!print *, "ENSTROPHY_STIR", Dyn%Grid%enstrophy_stir(22,47,future)
Dyn%Grid%enstrophy_robdamp(:,:,future) = Dyn%Grid%vorrobdamp(:,:,future)*(vorprime_prev+vorprime)
...
dt_vorkII(1:ie/2,:) = fft_grid_to_fourier(Dyn%grid%dt_vor(:,:,future))
dt_vorkII(ie/2+1:ie,:) = conjg(dt_vorkII(ie/2+1:2:-1,:))
!print *, "DT_VORKII", dt_vorkII(6,47)
...
!vor_xk(1:ie/2,:) = fft_grid_to_fourier(Dyn%Grid%vor_x(:,:,future)) 
!vor_xk(ie/2+1:ie,:) = conjg(vor_xk(ie/2+1:2:-1,:))
!vor_yk(1:ie/2,:) = fft_grid_to_fourier(Dyn%Grid%vor_y(:,:,future)) 
...
Dyn%grid%tester(1:ie/2+1,:,future) = ie**2*real(sum(sum(enstrophy_rvorprime_adveck(:,:,1:ie/2+1,:),1),1))
Dyn%grid%tester(ie/2+2:ie,:,future)= ie**2*aimag(sum(sum(enstrophy_rvorprime_adveck(:,:,2:ie/2,:),1),1))

! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!vorfluxBf_curr(:,:) = fft_grid_to_fourier(vprime_curr(:,:))*conjg(fft_grid_to_fourier(vorprime_curr(:,:)))*Dyn%grid%beta_star(:,:,current) ! gives enstrophy truncation term in spectral space
!enstrophy_truncf(:,:) = fft_grid_to_fourier(enstrophytrunc_prime(:,:))*conjg(fft_grid_to_fourier(vorprime_curr(:,:))) ! gives enstrophy truncation term in spectral space
!enstrophy_stirf(:,:) = fft_grid_to_fourier(enstrophystir_prime(:,:))*conjg(fft_grid_to_fourier(vorprime_curr(:,:))) ! gives enstrophy truncation term in spectral space
!enstrophy_lindamp_mf(:,:) = fft_grid_to_fourier(lindamp_m(:,:))*conjg(fft_grid_to_fourier(vorprime_curr(:,:))) ! gives enstrophy truncation term in spectral space
!
!Dyn%grid%fourier_enstrophy(1:ie/2+1,:,future) = real(enstrophyf(:,:))
!Dyn%grid%fourier_enstrophy(ie/2+2:ie,:,future) = aimag(enstrophyf(2:ie/2-1,:))
!print *, "fourier_enstrophy", enstrophyf(1:10,1)
...
! ZONAL MOMENTUM
!print *, "BUDGET: UTEND_AFTER = VQ - UTRUNC", Dyn%Grid%utend(40,40,future) - (Dyn%Grid%vq(40,40,future) - Dyn%Grid%dx_u2_v2(40,40,future) - Dyn%grid%utrunc(40,40,future)) ! BUDGET WITH NO DAMPING (SET COEFFICENTS TO ZERO IN NAMELIST)
!print *, "BUDGET: UTEND_AFTER = VQ - UTRUNC - LIND + SPECD + STIR + ROBD", Dyn%Grid%utend(40,40,future) - (Dyn%Grid%vq(40,40,future) - Dyn%grid%utrunc(40,40,future) - Dyn%Grid%ulindamp_m(40,40,future) &
!- Dyn%Grid%ulindamp_e(40,40,future) + Dyn%grid%uspecdamp(40,40,future) + Dyn%grid%ustir(40,40,future) + Dyn%grid%urobdamp(40,40,future))
! MERIDIONAL MOMENTUM
!print *, "BUDGET: VTEND = -UQ - VTRUNC", Dyn%Grid%vtend(40,40,future) - (-Dyn%Grid%uq(40,40,future) - Dyn%Grid%dy_u2_v2(40,40,future) - Dyn%grid%vtrunc(40,40,future)) 
...
Dyn%grid%vorlindamp_e(22,47,future) - Dyn%grid%vorlindamp_m(22,47,future) + Dyn%Grid%vorspecdamp(22,47,future) + Dyn%Grid%vorstir(22,47,future) + Dyn%Grid%vorrobdamp(22,47,future))
! ENERGY
!print *, "ENERGY_TEND = VORADVEC - GRADTERM - TRUNC - LINDAMP + SPECDAMP + STIR + ROBDAMP", Dyn%Grid%energy_tend(40,40,future) - (Dyn%Grid%energy_voradvec(40,40,future) & 
! - Dyn%Grid%energy_gradterm(40,40,future) - Dyn%Grid%energy_lindamp_m(40,40,future) - Dyn%Grid%energy_lindamp_e(40,40,future) - Dyn%Grid%energy_trunc(40,40,future) + Dyn%Grid%energy_specdamp(40,40,future)&
! + Dyn%Grid%energy_stir(40,40,future) + Dyn%Grid%energy_robdamp(40,40,future))
! MEAN ENERGY
!print *, "ENERGY_TEND_MEAN = VORADVEC - GRADTERM - TRUNC - LINDAMP + SPECDAMP + STIR + ROBDAMP", Dyn%Grid%energy_tend_mean(40,40,future) - (Dyn%Grid%energy_voradvec_mean(40,40,future) & 
! - Dyn%Grid%energy_gradterm_mean(40,40,future) - Dyn%Grid%energy_lindamp_m_mean(40,40,future) - Dyn%Grid%energy_lindamp_e_mean(40,40,future) - Dyn%Grid%energy_trunc_mean(40,40,future) & 
! + Dyn%Grid%energy_specdamp_mean(40,40,future) + Dyn%Grid%energy_stir_mean(40,40,future) + Dyn%Grid%energy_robdamp_mean(40,40,future))
! ENSTROPHY
...
!    call read_data('INPUT/barotropic_dynamics.res.nc', 'fourier_enstrophy', Dyn%Grid%fourier_enstrophy(:,:,nt), grid_domain, timelevel=nt)
    if(Dyn%spec_tracer) then
...
!  call write_data('RESTART/barotropic_dynamics.res.nc', 'fourier_enstrophy', Dyn%Grid%fourier_enstrophy(:,:,nn), grid_domain)
  if(Dyn%spec_tracer) then
