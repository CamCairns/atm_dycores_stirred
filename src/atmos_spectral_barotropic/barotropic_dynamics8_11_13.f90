...
beta_star=>NULL(), vormean_x=>NULL(), vormean_y=>NULL(), vorprime_x=>NULL(), vorprime_y=>NULL(), vormean_y_tend=>NULL(), vor_y=>NULL(), vor_x=>NULL(), & 
...
vortendk=>NULL(), vormeantend=>NULL(), vorprimetend=>NULL(), I_XX=>NULL(), v_y=>NULL(), u_dvordx=>NULL(), v_dvordy=>NULL(), vor_dudx=>NULL(), vor_dvdy=>NULL(),&
! Some more variables
uprev=>NULL(), ucurr=>NULL(), vprev=>NULL(), vcurr=>NULL(), vorprev=>NULL(), vorcurr=>NULL(), rvor_adveck=>NULL(), &
tester=>NULL(), tester2=>NULL(), tester3=>NULL(), tester4=>NULL(), tester5=>NULL(), tester6=>NULL(), &
! Energy
...
namelist /barotropic_dynamics_nml/ check_fourier_imag, south_to_north, &
                                 ...
                                 eddy_lat, eddy_width, zeta_0, m_0, damp, spec_deriv, mult, linmult, tau, tau1, spec_calc, read_file, &
                                 valid_range_v
...
allocate (Dyn%Grid(1,1)%tester4			(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid(1,1)%tester5			(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid(1,1)%tester6			(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid(1,1)%I_XX			(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid(1,1)%rvor_adveck		(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid(1,1)%u_dvordx		(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid(1,1)%v_dvordy		(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid(1,1)%vor_dudx		(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid(1,1)%vor_dvdy		(is:ie, js:je, num_time_levels))
...
! V_Y
 call vor_div_from_uv_grid (-Dyn%Grid(1,1)%v(:,:,current), zerog, dt_vors_rub, dt_divs_rub)  ! CHANGE BACK TO V WHEN FINISHED!!!!
...
! call vor_div_from_uv_grid (-f_array*Dyn%Grid(1,1)%v(:,:,current), f_array*Dyn%Grid(1,1)%u(:,:,current), dt_vors_rub, dt_divs_rub) 
! call trans_spherical_to_grid(dt_vors_rub, Dyn%Grid(1,1)%tester6(:,:,future))
! call vor_div_from_uv_grid (-Dyn%Grid(1,1)%pv(:,:)*Dyn%Grid(1,1)%v(:,:,current), Dyn%Grid(1,1)%pv(:,:)*Dyn%Grid(1,1)%u(:,:,current), dt_vors_rub, dt_divs_rub) 
! call trans_spherical_to_grid(dt_vors_rub, Dyn%Grid(1,1)%tester(:,:,future))
! 1st TERM RHS
! call vor_div_from_uv_grid (-Dyn%Grid(1,1)%vor(:,:,current), zerog, dt_vors_rub, dt_divs_rub) 
...
! 3rd TERM
! call vor_div_from_uv_grid (zerog, Dyn%Grid(1,1)%vor(:,:,current), dt_vors_rub, dt_divs_rub) 
! call trans_spherical_to_grid(dt_vors_rub, Dyn%Grid(1,1)%tester4(:,:,future))
! Dyn%Grid(1,1)%tester4(:,:,future) = Dyn%Grid(1,1)%u(:,:,current)*Dyn%Grid(1,1)%tester4(:,:,future)
! 4th TERM
! call vor_div_from_uv_grid (zerog, Dyn%Grid(1,1)%u(:,:,current), dt_vors_rub, dt_divs_rub) 
! call trans_spherical_to_grid(dt_vors_rub, Dyn%Grid(1,1)%tester5(:,:,future))
! Dyn%Grid(1,1)%tester5(:,:,future) = Dyn%Grid(1,1)%vor(:,:,current)*Dyn%Grid(1,1)%tester5(:,:,future)
! 4th TERM
! call vor_div_from_uv_grid (-f_array, zerog, dt_vors_rub, dt_divs_rub) 
! call trans_spherical_to_grid(dt_vors_rub, Dyn%Grid(1,1)%tester6(:,:,future))
! Dyn%Grid(1,1)%tester6(:,:,future) = Dyn%Grid(1,1)%v(:,:,current)*Dyn%Grid(1,1)%tester6(:,:,future)
! Mathematically tester = tester2 + tester3 + tester4 + tester5
!
...
! ---------  CALCULATING DT_VOR TWO DFFERENT WAYS ----------------------
! This is the line of code in the model that does it all. It calculates dt_vor. There are problems with calculating the spectral triple eddy product with dt_vor calculated in its original way. One way of 
!resolving this is to calculate it in a different (though in principle, mathematically equivalent way). Include a toggle to change how it is calculated.
!
! Set spec_deriv == 0 for original model differentiation method
! Set spec_deriv =/= 0 for product rule differentiation method
!
...
! call trans_spherical_to_grid(dt_vors,Dyn%Grid(1,1)%tester(:,:,future))
...
! dU_dX
 call vor_div_from_uv_grid (zerog, Dyn%Grid(1,1)%u(:,:,current), vors_rub, dt_divs_rub)
! call vor_div_from_uv_grid (zerog, Dyn%Grid(1,1)%u(:,:,current), dt_vors_rub, dt_divs_rub) 
 call trans_spherical_to_grid(vors_rub, Dyn%Grid(1,1)%vor_dudx(:,:,future))
 Dyn%Grid(1,1)%vor_dudx(:,:,future) = Dyn%Grid(1,1)%vor(:,:,current)*Dyn%Grid(1,1)%vor_dudx(:,:,future)
! dV_dY
 call vor_div_from_uv_grid (-Dyn%Grid(1,1)%v(:,:,current), zerog, vors_rub, dt_divs_rub) 
! call vor_div_from_uv_grid (-Dyn%Grid(1,1)%v(:,:,current), zerog, dt_vors_rub, dt_divs_rub) 
 call trans_spherical_to_grid(vors_rub, Dyn%Grid(1,1)%vor_dvdy(:,:,future))
 Dyn%Grid(1,1)%vor_dvdy(:,:,future) = Dyn%Grid(1,1)%vor(:,:,current)*Dyn%Grid(1,1)%vor_dvdy(:,:,future)
! dVOR_dX
 call vor_div_from_uv_grid (zerog, Dyn%Grid(1,1)%vor(:,:,current), vors_rub, dt_divs_rub)  
! call vor_div_from_uv_grid (zerog, Dyn%Grid(1,1)%vor(:,:,current), dt_vors_rub, dt_divs_rub) 
 call trans_spherical_to_grid(vors_rub, Dyn%Grid(1,1)%u_dvordx(:,:,future)) 
 Dyn%Grid(1,1)%u_dvordx(:,:,future) = Dyn%Grid(1,1)%u(:,:,current)*Dyn%Grid(1,1)%u_dvordx(:,:,future)
! dVOR_dY
 call vor_div_from_uv_grid (-Dyn%Grid(1,1)%vor(:,:,current), zerog, vors_rub, dt_divs_rub)
! call vor_div_from_uv_grid (-Dyn%Grid(1,1)%vor(:,:,current), zerog, dt_vors_rub, dt_divs_rub) 
 call trans_spherical_to_grid(vors_rub, Dyn%Grid(1,1)%v_dvordy(:,:,future))
 Dyn%Grid(1,1)%v_dvordy(:,:,future) = Dyn%Grid(1,1)%v(:,:,current)*Dyn%Grid(1,1)%v_dvordy(:,:,future)

 call trans_grid_to_spherical(Dyn%Grid(1,1)%u_dvordx(:,:,future) + Dyn%Grid(1,1)%vor_dudx(:,:,future) + Dyn%Grid(1,1)%v_dvordy(:,:,future) + Dyn%Grid(1,1)%vor_dvdy(:,:,future) + &
				Dyn%Grid(1,1)%v(:,:,current)*Dyn%Grid(1,1)%beta(:,:,future),dt_vors)
 dt_vors = -2*dt_vors
! call trans_spherical_to_grid(dt_vors,Dyn%Grid(1,1)%tester6(:,:,future))
 call uv_grid_from_vor_div(dt_vors,  zeros, Dyn%Grid(1,1)%dx_u2_v2(:,:,current), Dyn%Grid(1,1)%dy_u2_v2(:,:,current))
 Dyn%Grid(1,1)%dx_u2_v2(:,:,future) = Dyn%Tend%u - Dyn%Grid(1,1)%dx_u2_v2(:,:,current) - Dyn%Grid(1,1)%utrunc(:,:,future)
 Dyn%Grid(1,1)%dy_u2_v2(:,:,future) = Dyn%Tend%v - Dyn%Grid(1,1)%dy_u2_v2(:,:,current) - Dyn%Grid(1,1)%vtrunc(:,:,future)
endif
...
!print *, "DT_VORS", dt_vors(1:10,35)
!print *, "DT_VORS2", dt_vors2(1:10,35)
...
!print*, "UTEND", Dyn%Grid(1,1)%utend(10,10,future)
...
!print *, "VTEND", Dyn%Grid(1,1)%vtend(27,50,future)
