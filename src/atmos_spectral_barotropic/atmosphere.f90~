...
!use  barotropic_diagnostics_testing_mod, only: barotropic_diagnostics_testing_init,   &
!                                       barotropic_diagnostics_testing
...
! version information 
!========================================================================
character(len=128) :: version = '$Id: atmosphere.f90,v 10.0 2003/10/24 22:00:57 fms Exp $'
...
! call barotropic_diagnostics_testing_init(Time, num_lon, num_lat, id_lon, id_lat, id_lonb, id_latb)
...
current    = future
 ...
 call barotropic_diagnostics (Time+Time_step, Dyn%Grid, Phys, current)
! call barotropic_diagnostics (Time+Time_step, Dyn%Grid, Dyn%Testing, Phys, current)
! call barotropic_diagnostics_testing (Time+Time_step, Dyn%Testing, Phys, current)

!enstrophy = area_weighted_global_mean(Dyn%Grid(1,1)%vortend(:,:,current)*Dyn%Grid(1,1)%vortend(:,:,previous))
enstrophy = area_weighted_global_mean(Dyn%Grid(1,1)%vor(:,:,current)*Dyn%Grid(1,1)%vor(:,:,previous))

energy = &
- area_weighted_global_mean(Dyn%Grid(1,1)%stream*Dyn%Grid(1,1)%vor(:,:,previous))

if(root) then
  call get_time(Time+Time_step, second, day)
  if(mod(second+86400*day, print_interval) < dt) then
    write(*,1000) day, second, energy, enstrophy
  end if
end if
1000 format(1x, 'day =',i6,2x,'second =', i6, &
    2x,'energy = ',e13.6,3x,'enstrophy = ',e13.6)
