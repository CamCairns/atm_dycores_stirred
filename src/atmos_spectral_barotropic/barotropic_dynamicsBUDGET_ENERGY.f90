...
! Energy
umean_squared=>NULL(), uprime_squared=>NULL(), v_squared=>NULL(), energy=>NULL(), energydomain=>NULL(), energydomain_tend=>NULL(), energy_tend=>NULL(), energy_voradvec=>NULL(), energy_gradterm=>NULL(), &
energy_trunc=>NULL(), energy_lindamp_m=>NULL(), energy_lindamp_e=>NULL(), energy_specdamp=>NULL(), energy_stir=>NULL(), energy_robdamp=>NULL(), &
! Joe variables
...
allocate (Dyn%Grid%umean_squared	(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid%uprime_squared	(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid%v_squared		(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid%energy		(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid%energydomain		(is:ie, js:je, num_time_levels))
allocate (Dyn%Grid%energydomain_tend	(is:ie, js:je, num_time_levels))
...
real,    dimension(is:ie, js:je)  :: dt_vorg, stir, umean, uprime, beta, vormean, vorprime, energyprev
real,    dimension(is:ie, js:je)  :: uprev, vprev, vorprev, zerog, rob_v_diff, rob_u_diff, rob_vorg_diff, energytendcheck
integer :: j
...
energyprev = Dyn%Grid%energy(:,:,previous)
...
! ------------------------------------- U'^2,V'^2, ENERGY ---------------------------------
do j = js, je
 umean(:,j)= sum(Dyn%Grid%u(:,j,future))/count(Dyn%Grid%u(:,j,future) > -10000) ! Calculate zonal mean, zonal flow (needed for linear damping)
end do
uprime = Dyn%Grid%u(:,:,future)-umean(:,:)

Dyn%Grid%umean_squared(:,:,future) = umean**2
Dyn%Grid%uprime_squared(:,:,future) = uprime**2
Dyn%Grid%v_squared(:,:,future) = Dyn%Grid%v(:,:,future)**2

!print *, "U MEAN SQUARED", Dyn%Grid%umean_squared(10,10,future)
!print *, "U PRIME SQUARED", Dyn%Grid%uprime_squared(10,10,future)
!print *, "V (PRIME) SQUARED", Dyn%Grid%v_squared(10,10,future)

! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

! -------------------------------- U,V,VOR & ENERGY TENDENCIES ------------------------------------
if(previous == current) then
...
energytendcheck(:,:) = (Dyn%Grid%energy(:,:,future)-energyprev)/delta_t
endif
...
!============ COMMENT ON U^2 TENDENCY CALCULATION ==================
! Interestingly u2tend not= 2*u*utend as would be expected using product rule.
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
print *, "ENERGYTENDCHECK", energytendcheck(10,10)*2
