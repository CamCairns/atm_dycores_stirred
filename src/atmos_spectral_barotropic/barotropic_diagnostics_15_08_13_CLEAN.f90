...
integer :: id_enstrophy_tendk, id_enstrophy_pvor_adveck, id_enstrophy_rvormean_adveck, id_enstrophy_trunck, id_enstrophy_lindamp_ek, id_enstrophy_lindamp_mk, id_enstrophy_specdampk, id_enstrophy_robdampk, id_enstrophy_stirk, id_enstrophy_rvorprime_adveck_SUM, id_I(1:12,1:12)
...
id_I					= register_diag_field(mod_name, 'I',axis_2d, Time, 'vor with an fft taken along lon dim'	,'m/s'		)
!id_I    				= register_diag_field(mod_name, 'I',axis_3d, Time, 'I'	,'1/s'		)
!id_enstrophy_rvorprime_adveck		= register_diag_field(mod_name, 'enstrophy_rvorprime_adveck',axis_3d, Time, 'vor with an fft taken along lon dim'	,'m/s'		)

!print *, Dyn%Testing%enstrophy_rvorprime_adveck
...
if(id_vorprime_curr > 0) used = send_data(id_vorprime_curr , Grid%vorprime_curr(:,:,time_index) , time)
...
if(id_vormeantend > 0) used = send_data(id_vormeantend , Grid%vormeantend(:,:,time_index) , time)
if(id_vorprimetend > 0) used = send_data(id_vorprimetend , Grid%vorprimetend(:,:,time_index) , time)
if(id_enstrophy_rvormean_adevc_Matlab > 0) used = send_data(id_enstrophy_rvormean_adevc_Matlab , Grid%enstrophy_rvormean_adevc_Matlab(:,:,time_index) , time)
if(id_enstrophy_rvorprime_adevc_Matlab > 0) used = send_data(id_enstrophy_rvorprime_adevc_Matlab , Grid%enstrophy_rvorprime_adevc_Matlab(:,:,time_index) , time)
...
if(id_enstrophy_rvorprime_adveck_SUM > 0) used = send_data(id_enstrophy_rvorprime_adveck_SUM , Grid%enstrophy_rvorprime_adveck_SUM(:,:,time_index) , time)

do p=1,64
   do q = 1,64
       if(id_I(p,q) > 0) used = send_data(id_I , Testing(p,q)%I(:,:,time_index) , time)
   do
do
...
!print *, "ENSTIRK", Grid%enstrophy_stirk(22,22,time_index)
