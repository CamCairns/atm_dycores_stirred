...
type grid_type
   real, pointer, dimension(:,:,:) :: u=>NULL(), utend=>NULL(), vvor_beta=>NULL(), source=>NULL(), source_beta=>NULL(), sink=>NULL(), sink_beta=>NULL(), dens_dt=>NULL(), robertssink=>NULL(), ensflux=>NULL(), ensflux_div=>NULL(),ensflux_div_beta=>NULL(), vtend=>NULL(), vortend=>NULL(), v=>NULL(), vor=>NULL(), trs=>NULL(), tr=>NULL()
...
if(read_file == 1) then
	open (unit=1, file='/scratch/z3309205/barotropic/u.txt', status='old', action='read')
	read (1,*) icnt
...
real,    dimension(is:ie, js:je)  :: dt_vorg, stir, umean, beta, vormean, roberts, vorprime
integer :: j
...
! want to add a linear damping term on the zonal-mean.  This means just add the same number at each longitude, which is 
do j = js, je
umean(:,j)= sum(Dyn%Grid%u(:,j,current))/count(Dyn%Grid%u(:,j,current) > -10000) 
dt_vorg(:,j)= sum(Dyn%Tend%u(:,j))/count(Dyn%Grid%u(:,j,current) > -10000) 
end do
Dyn%Tend%u=Dyn%Tend%u+linmult*dt_vorg

if(damp > 1.0) then
Dyn%Grid%vtend(:,:,current)= - Dyn%Grid%v(:,:,current)/tau  
!Dyn%Grid%utend(:,:,current)= - (Dyn%Grid%u(:,:,current))/tau  
Dyn%Grid%utend(:,:,current)= - (Dyn%Grid%u(:,:,current)-umean)/tau  
Dyn%Tend%u  = Dyn%Tend%u - umean/tau1
Dyn%Tend%u = Dyn%Tend%u + Dyn%Grid%utend(:,:,current)
Dyn%Tend%v = Dyn%Tend%v + Dyn%Grid%vtend(:,:,current)
endif

!add on the eddy damping




call vor_div_from_uv_grid (Dyn%Grid%utend(:,:,current), Dyn%Grid%vtend(:,:,current), damped, dt_divs_throwaway)
call vor_div_from_uv_grid (Dyn%Tend%u, Dyn%Tend%v, dt_vors, dt_divs)



dt_vors_prev = dt_vors

if(damp > 0.0) then
call compute_spectral_damping(Dyn%Spec%vor(:,:,previous), dt_vors, delta_t)
endif

dt_vors_prev = dt_vors - dt_vors_prev

damped = damped + dt_vors_prev




call trans_spherical_to_grid(damped, Dyn%Grid%vortend(:,:,current))


!call trans_spherical_to_grid(dt_vors_prev,damped)
!call uv_grid_from_vor_div  (dt_vors_prev,  zeros, udamp(:,:),  vdamp(:,:))


!Dyn%Grid%utend(:,:,current) = Dyn%Grid%utend(:,:,current) + udamp(:,:)
!Dyn%Grid%vtend(:,:,current) = Dyn%Grid%vtend(:,:,current) + vdamp(:,:)
!Dyn%Grid%vtend(:,:,current) = umean
...
!calculate the source term in frid space
call trans_spherical_to_grid(dt_vors_prev, stir)
...
call trans_spherical_to_grid(Dyn%Spec%vor(:,:,current), dt_vorg)
roberts=dt_vorg-roberts
