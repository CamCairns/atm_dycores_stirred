...
TYPE(testing_type), ALLOCATABLE, DIMENSION(:,:) :: testing
...
allocate(testing(ie/2,ie/2))
do p = 1,12! ie/2
 do q = 1,12! ie/2 
   allocate(Dyn%testing(p,q)%I(is:ie, js:je, num_time_levels))
 end do
end do
...
do p=1,12!size(kmat)/2
       do q=1,12!size(kmat)/2
	    Dyn%testing(p,q)%I(1:ie/2+1,:,future) = ie**2*real(enstrophy_rvorprime_adveck(p,q,1:ie/2+1,:))!
	    Dyn%testing(p,q)%I(ie/2+2:ie,:,future) = ie**2*aimag(enstrophy_rvorprime_adveck(p,q,2:ie/2,:))
       end do
end do
