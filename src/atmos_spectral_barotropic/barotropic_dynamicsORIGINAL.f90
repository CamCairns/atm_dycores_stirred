...
real,    dimension(is:ie, js:je)  :: dt_vorg, stir, umean, beta, vormean, roberts, vorprime, udamp, vdamp
integer :: j
...
Dyn%Tend%u = Dyn%Tend%u + Dyn%Grid%utend(:,:,current) !add on the eddy damping
Dyn%Tend%v = Dyn%Tend%v + Dyn%Grid%vtend(:,:,current)
endif

call vor_div_from_uv_grid (Dyn%Grid%utend(:,:,current), Dyn%Grid%vtend(:,:,current), damped, dt_divs_throwaway) ! Linear damping  due to eddies
call vor_div_from_uv_grid (Dyn%Tend%u, Dyn%Tend%v, dt_vors, dt_divs) ! Total linear damping
...
!calculate the source term in grid space
