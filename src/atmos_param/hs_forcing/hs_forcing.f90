...
!-------------------- namelist -----------------------------------------
   ...
   real :: P00 = 1.e5, mag=0., latcentre=0.,horwidth=0., height=0., verwidth=0.
...
!-----------------------------------------------------------------------
   ...
   namelist /hs_forcing_nml/  no_forcing, t_zero, t_strat, delh, delv, eps, &
                              ...
                              trflux, trsink, mag, latcentre, horwidth, height, verwidth
...
!     rayleigh damping of wind components near the surface
      ...
      call rayleigh_damping ( lat, ps, p_full, u, v, utnd, vtnd, mask=mask )
...
!#######################################################################
 ...
 subroutine rayleigh_damping ( lat, ps, p_full, u, v, udt, vdt, mask )
...
real, dimension(size(u,1),size(u,2)) :: sigma, vfactr, rps, torque
...
!----------------compute damping----------------------------------------
      ...
      rps = 1./ps
         ...
         torque  (:,:) = mag*exp(-(((latcentre*3.14/180)-lat(:,:))*((latcentre*3.14/180)-lat(:,:)))/((horwidth*3.14/180)*(horwidth*3.14/180)))

torque  (:,:) = torque(:,:)+mag*exp(-(((-latcentre*3.14/180)-lat(:,:))*((-latcentre*3.14/180)-lat(:,:)))/((horwidth*3.14/180)*(horwidth*3.14/180)))
...
!udt(:,:,k) = udt(:,:,k)+1/(24*3600)
udt(:,:,k) = udt(:,:,k)+torque(:,:)*exp(-((height-sigma(:,:))*(height-sigma(:,:)))/(verwidth*verwidth))
!udt(:,:,k) = udt(:,:,k)+torque(:,:)
!udt(:,:,k)=udt(:,:,k)+mag
      enddo
