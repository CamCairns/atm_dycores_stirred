...
complex, dimension(ms:me, ns:ne, num_levels  ) :: vors_prev, vors_post, vors_cur, divs_prev, divs_post, divs_cur, vors_rob, divs_rob
...
real, dimension(is:ie, js:je, num_levels             ) :: vorg_post, divg_post
!declare variables needed
real, dimension(is:ie, js:je, num_levels             ) :: dt_ug_rub,dt_ug_rub1, dt_ug_in, dt_ug_timestep, dt_ug_sigma, dt_ug_pgrad, dt_ug_vertadvec,dt_ug_vertadvec_eddy , wtheta,  dt_ug_vorflux, dt_ug_vorfluxm, dt_ug_vorfluxf, dt_ug_out, dt_ug_int, dt_ug_trunc, vtheta, eke
real, dimension(is:ie, js:je, num_levels             ) :: dt_vg_rub,dt_vg_rub1, dt_vg_in, dt_vg_timestep, dt_vg_sigma, dt_vg_pgrad, dt_vg_vertadvec, dt_vg_vorflux, dt_vg_out, dt_vg_int, dt_vg_trunc
...
!used to output incoming value
dt_ug_in = dt_ug_tmp
dt_vg_in = dt_vg_tmp

!"rubish" values, given dt_ug_tmp value at this point, in order to subtract below
dt_ug_rub = dt_ug_tmp
dt_vg_rub = dt_vg_tmp
...
!values returned from four_inOone, minus the "rubish" values above, give change withing four_in_one subroutine
dt_ug_sigma = dt_ug_tmp - dt_ug_rub
dt_vg_sigma = dt_vg_tmp - dt_vg_rub
...
!change due to vertical advection subroutine
dt_ug_vertadvec = dt_ug_tmp - dt_ug_rub

do k=1,num_levels
  do j = js,je
vtheta(:,j,k) =  (vg(:,j,k,current) - sum(vg(:,j,k,current))/size(vg(:,j,k,current)))*(tg(:,j,k,current) - sum(tg(:,j,k,current))/size(tg(:,j,k,current)))*(p_full(:,j,k)/1000)**(rdgas/cp_air)
wtheta(:,j,k) =  (wg_full(:,j,k) - sum(wg_full(:,j,k))/size(wg_full(:,j,k)))*(tg(:,j,k,current) - sum(tg(:,j,k,current))/size(tg(:,j,k,current)))*(p_full(:,j,k)/1000)**(rdgas/cp_air)
...
!change due to vertical advection subroutine
dt_vg_vertadvec = dt_vg_tmp - dt_vg_rub
...
dt_ug_vorflux(:,j,k) = (vorg(:,j,k) - sum(vorg(:,j,k))/size(vorg(:,j,k)))*(vg(:,j,k,current) - sum(vg(:,j,k,current))/size(vg(:,j,k,current)))
dt_ug_vorfluxm(:,j,k) = (sum(vorg(:,j,k))/size(vorg(:,j,k)))*(sum(vg(:,j,k,current))/size(vg(:,j,k,current)))
dt_ug_vorfluxf(:,j,k) = coriolis(j)*sum(vg(:,j,k,current))/size(vg(:,j,k,current))
eke(:,j,k) = (vg(:,j,k,current) - sum(vg(:,j,k,current))/size(vg(:,j,k,current)))*(vg(:,j,k,current) - sum(vg(:,j,k,current))/size(vg(:,j,k,current))) + (ug(:,j,k,current) - sum(ug(:,j,k,current))/size(ug(:,j,k,current)))*(ug(:,j,k,current) - sum(ug(:,j,k,current))/size(ug(:,j,k,current)))
  enddo
enddo
!"rubish" change due to vorticity flux advection (i.e. Corriolis pluss momentum flux convergence)
!dt_ug_vorflux = dt_ug_tmp - dt_ug_rub
dt_vg_vorflux = dt_vg_tmp - dt_vg_rub
...
call uv_grid_from_vor_div    (dt_vors, dt_divs, dt_ug_trunc, dt_vg_trunc)
!differrence that result from transfering to spectral space and back
dt_ug_trunc = dt_ug_trunc - dt_ug_rub
dt_vg_trunc = dt_vg_trunc - dt_vg_rub

!"rubish" values, given dt_ug_tmp value at this point, in order to subtract below
call uv_grid_from_vor_div    (dt_vors, dt_divs, dt_ug_rub, dt_vg_rub)
...
call uv_grid_from_vor_div    (dt_vors, dt_divs, dt_ug_pgrad, dt_vg_pgrad)


!difference that resulted from compute_laplacian subroutine (pressure gradient)
dt_ug_pgrad = dt_ug_pgrad-dt_ug_rub
dt_vg_pgrad = dt_vg_pgrad-dt_vg_rub

!get value here , in order to get change post-upcoming subroutine
call uv_grid_from_vor_div    (dt_vors, dt_divs, dt_ug_int, dt_vg_int)
if(use_implicit) call implicit_correction (dt_divs, dt_ts, dt_ln_ps, divs, ts, ln_ps, delta_t, previous, current)
call uv_grid_from_vor_div    (dt_vors, dt_divs, dt_ug_rub, dt_vg_rub)
!these values are the result of implicit_correction subroutine
dt_ug_int = dt_ug_rub-dt_ug_int
dt_vg_int = dt_vg_rub-dt_vg_int
...
call uv_grid_from_vor_div    (dt_vors, dt_divs, dt_ug_out, dt_vg_out)
!these values are due to the spectral damping (dt_ug_rub was calculated imedietly before call to camping)
dt_ug_damp = dt_ug_out - dt_ug_rub
dt_vg_damp = dt_vg_out - dt_vg_rub


call trans_spherical_to_grid (dt_ts_damp, dt_tg_damp)

!these are going to be used to calculate the timestep damping, manually, after timestep call
vors_prev = vors(:,:,:,previous)
divs_prev = divs(:,:,:,previous)
vors_cur = vors(:,:,:,current)
divs_cur = divs(:,:,:,current)
...
vors_post = vors(:,:,:,future)
divs_post = divs(:,:,:,future)
!this is what the roberts filter does elsewhere
vors_rob = robert_coeff*(vors_post+vors_prev - 2*vors_cur)
divs_rob = robert_coeff*(divs_post+divs_prev - 2*divs_cur)

!back to u v on grid
call uv_grid_from_vor_div(vors_rob, divs_rob, dt_ug_timestep, dt_vg_timestep)
dt_ug_timestep=dt_ug_timestep/3600
dt_vg_timestep=dt_vg_timestep/3600

!call trans_spherical_to_grid (dt_vors, vorg_post)


vorg_post = vorg
divg_post = divg
call trans_spherical_to_grid(dt_divs, divg_post)
call trans_spherical_to_grid(dt_vors, vorg_post)
...
!make sure these are recieved in teh same order they're passed ...
call every_step_diagnostics( &
     Time_diag, psg(:,:,current), ug(:,:,:,current), vg(:,:,:,current), tg(:,:,:,current), grid_tracers(:,:,:,:,:), vtheta, wtheta, &
     dt_ug_damp, dt_ug_in, dt_ug_sigma, dt_ug_pgrad, dt_ug_vertadvec,  dt_ug_vorflux, dt_ug_vorfluxm, dt_ug_vorfluxf, dt_ug_out, dt_ug_int, dt_ug_timestep, dt_ug_trunc, dt_vg_damp, dt_vg_in, dt_vg_sigma, dt_vg_pgrad, dt_vg_vertadvec, dt_vg_vorflux, dt_vg_out, dt_vg_int, dt_vg_timestep, dt_vg_trunc,  dt_tg_damp, vorg_post, divg_post, eke, current)
