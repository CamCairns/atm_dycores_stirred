...
character(len=128), parameter :: version = '$Id: spectral_damping.f90,v 13.0 2006/03/28 21:18:04 fms Exp $'
...
subroutine compute_spectral_damping_3d(spec, dt_spec, current_dt, dt_spec_damp) 

complex, intent(in), dimension(:,:,:) :: spec
real,    intent(in) :: current_dt
complex, intent(inout), dimension (:,:,:) :: dt_spec
complex, intent(out), dimension(:,:,:), optional :: dt_spec_damp
...
if(present(dt_spec_damp)) then
  dt_spec_damp = dt_spec
endif
...
if(present(dt_spec_damp)) then
  dt_spec_damp = dt_spec - dt_spec_damp
endif
...
if(present(dt_vor_damp)) then
  dt_vor_damp = dt_vor - dt_vor_damp
endif
...
if(present(dt_div_damp)) then
  dt_div_damp = dt_div - dt_div_damp
endif

return
end subroutine compute_spectral_damping_div
!-----------------------------------------------------------------
subroutine compute_spectral_damping_2d(spec, dt_spec, current_dt, dt_spec_damp)  
